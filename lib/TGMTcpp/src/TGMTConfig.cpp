...
bool TGMTConfig::LoadSettingFromFile(std::string settingFile)
{
	...
	return m_reader.LoadSettingFromFile(settingFile);
}
...
TGMT_STRING TGMTConfig::ReadSettingFromConfig(std::string iniFile, std::string section, std::string key)
{
	...
	std::wstring winiFile = TGMTutil::String2WString(iniFile);
	...
	std::string result = "";
	char retStr[1024] = { 0 };
	DWORD sizeRead = GetPrivateProfileString(section.c_str(), key.c_str(), NULL,
...
{
	TGMT_STRING temp = ReadSettingFromConfig(m_iniFile, section, key);
#ifdef UNICODE
	if (temp != L"")
		return TGMTutil::WString2String( temp);
#else
	if (temp != "")
		return temp;
#endif
...
void TGMTConfig::WriteConfigString(std::string section, std::string key, TGMT_STRING value)
{
...
bool TGMTConfig::INIReader::LoadSettingFromFile(std::string settingFile)
{
	return 0 == ini_parse((char*)settingFile.c_str(), ValueHandler, this);
}

std::string TGMTConfig::INIReader::ReadValueString(std::string section, std::string name, std::string default_value)
{
